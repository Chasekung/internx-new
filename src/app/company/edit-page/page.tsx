...
const EditCompanyPage = () => {
  ...
  const checkSession = async () => {
    try {
      ...
      fetchCompanyData(session.access_token);
    } catch (error) {
  ...
  const fetchCompanyData = async (token: string) => {
    try {
      console.log('Fetching company data with token:', token);
      ...
      // Convert the flat company data into locations array
      const locationsList: Location[] = [];
      for (let i = 1; i <= 10; i++) {
        if (data[`address_${i}`]?.trim() || data[`city_${i}`]?.trim() || data[`state_${i}`]) {
          locationsList.push({
            address: data[`address_${i}`] || '',
            city: data[`city_${i}`] || '',
            state: data[`state_${i}`] || '',
            is_headquarters: data[`is_headquarters_${i}`] || false
          });
        }
      }
      
      // If no locations found, add one empty location
      if (locationsList.length === 0) {
        locationsList.push({ address: '', city: '', state: '', is_headquarters: false });
      }
      ...
      setMessage({ 
        type: 'error', 
        text: error instanceof Error ? error.message : 'Failed to load company data' 
      });
    }
  };
  ...
  const uploadLogo = async (token: string): Promise<string | undefined> => {
  ...
  const handleSave = async () => {
    try {
      ...
      if (companyLogo) {
        try {
          logoUrl = await uploadLogo(session.access_token);
        } catch (error) {
      ...
      // Convert locations to numbered fields
      const locationFields: Record<string, any> = {};
      
      // First, clear all location fields
      for (let i = 1; i <= 10; i++) {
        locationFields[`address_${i}`] = null;
        locationFields[`city_${i}`] = null;
        locationFields[`state_${i}`] = null;
        locationFields[`is_headquarters_${i}`] = null;
      }

      // Then set the active locations
      locations.forEach((location, index) => {
        const num = index + 1;
        if (num <= 10) {  // Ensure we don't exceed 10 locations
          locationFields[`address_${num}`] = location.address.trim() || null;
          locationFields[`city_${num}`] = location.city.trim() || null;
          locationFields[`state_${num}`] = location.state || null;
          locationFields[`is_headquarters_${num}`] = location.is_headquarters;
        }
      });

      // Prepare the data for update
      const cleanedData = {
        company_name: companyData.companyName.trim() || null,
        ...
        company_size: companyData.companySize.trim() || null,
      ...
      const profileResponse = await fetch('/api/companies/profile', {
        method: 'PUT',
        headers: {
      ...
      if (!profileResponse.ok) {
        ...
        throw new Error(responseData.error || 'Failed to update company profile');
      }
